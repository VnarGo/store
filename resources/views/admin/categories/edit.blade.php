                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.categories.update", [$category->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('subcategories') ? 'has-error' : '' }}">
                            <label class="required" for="subcategories">subcategories</label>
                            ...
                            <select class="form-control select2" name="subcategories[]" id="subcategories" multiple required>
                                @foreach($subcategories as $id => $subcategories)
                                    <option value="{{ $id }}" {{ (in_array($id, old('subcategories', [])) || $category->subcategories->contains($id)) ? 'selected' : '' }}>{{ $subcategories }}</option>
                                @endforeach
                            </select>
